...
const scrapeImages = async ({term, lang = 'en', engine = 'yandex', target = 3000}) => {
  ...
  let encodedUrl;
  let selector = 'img';
  let selectorsToImageUrlConverter = items => items.map(item => item.src);
  ...
  switch (engine) {
    ...
    case 'flickr':
      ...
      break;
    case 'baidu':
      encodedUrl = text => `https://image.baidu.com/search/index?tn=baiduimage&word=${encodeURIComponent('"' + text + '"')}`
      break;
    case 'sogou':
      encodedUrl = text => `https://pic.sogou.com/pics?query=${encodeURIComponent('"' + text + '"')}`
      break;
    case 'haosou':
      encodedUrl = text => `https://image.so.com/i?q=${encodeURIComponent('"' + text + '"')}&src=srp`
      break;
    case 'yahoo':
      encodedUrl = text => `https://images.search.yahoo.com/search/images?fr2=sb-top-images.search&p=${encodeURIComponent('"' + text + '"')}&ei=UTF-8&iscqry=&fr=sfp`
      moreResultsButtonSelector = 'button.more-res'
      break;
    default:
